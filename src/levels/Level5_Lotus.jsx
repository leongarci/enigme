...
import { Music, Film, Book, Flower2, MapPin, CheckCircle2, Play, RotateCcw, Heart, AlertTriangle, Keyboard } from 'lucide-react';

/**
 * NIVEAU 5 : L'ÉCHO DE L'ORCHESTRE
 * Version optimisée pour la réactivité immédiate à l'appui (keydown).
 */
export default function Level5Lotus({ onSolve }) {
    ...
    // Refs pour la logique de jeu haute performance
    ...
    const keysPressed = useRef({}); // Empêche le "key repeat" du clavier

    const TARGET_SCORE = 2000;
    const HIT_ZONE_Y = 85;
    const HIT_THRESHOLD = 12; // Fenêtre de tir légèrement plus large pour le confort
    ...
    const handleHit = useCallback((laneIndex) => {
        ...
        // On cherche la note la plus basse (plus grand Y) dans la zone pour éviter de valider une note trop haute
        ...
        if (targetNotes.length > 0) {
            ...
            if (distance < HIT_THRESHOLD) {
                ...
                // On met à jour l'état immédiatement pour le feedback visuel
    ...
    useEffect(() => {
        ...
        const loop = (time) => {
            ...
            // Nettoyage : On ne garde que les notes non touchées et encore à l'écran
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            if (keysPressed.current[e.code]) return; // Ignorer le key repeat
            keysPressed.current[e.code] = true;

            if (gameState === 'ready') {
                const isGameKey = lanes.some(l => l.code === e.code);
                if (isGameKey) {
                    startCountdown();
                    return;
                }
    ...
    useEffect(() => {
        ...
        if (segmentCount > revealedSegments.length) {
            setRevealedSegments(SEGMENTS.slice(0, segmentCount));
        }
    ...
    const renderOverlay = () => {
        if (gameState === 'ready') {
            return (
                <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm rounded-3xl animate-fadeIn">
                    <div className="bg-slate-900 border-2 border-amber-500 p-8 rounded-3xl text-center shadow-2xl max-w-sm">
                        <Keyboard className="text-amber-500 mx-auto mb-4" size={40} />
                        ...
                        <p className="text-slate-400 mb-6 text-xs leading-relaxed">
                            ...
                            L'audition commence dès la première note.
                        </p>
                        <p className="text-amber-500 font-black text-[10px] animate-pulse uppercase tracking-widest">
            ...
            return (
                <div className="absolute inset-0 z-[60] flex items-center justify-center bg-slate-950/90 backdrop-blur-md rounded-3xl animate-fadeIn">
                    <div className="text-center p-10">
                        ...
                        <p className="text-slate-400 mb-8 text-sm">Le rythme s'est brisé. Reprends ton souffle.</p>
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 animate-fadeIn select-none font-sans">
            ...
            <div className="text-center mb-6">
                <h2 className="text-3xl font-black text-amber-500 uppercase tracking-widest italic">
                    L'Écho de l'Orchestre
                </h2>
                <div className="flex justify-center gap-10 mt-4">
                    <div className="text-center">
                        <p className="text-[10px] text-slate-500 uppercase font-bold">Score</p>
                        <p className="text-xl font-mono text-white">{score.toLocaleString()}</p>
                    </div>
                    <div className="text-center">
                        <p className="text-[10px] text-slate-500 uppercase font-bold">Vies</p>
                        <div className="flex gap-1 mt-1 justify-center">
                            {[...Array(3)].map((_, i) => (
                                <Heart key={i} size={16} className={i < lives ? "fill-red-500 text-red-500" : "text-slate-800"} />
                            ))}
                        </div>
                    </div>
                    <div className="text-center">
                        <p className="text-[10px] text-slate-500 uppercase font-bold">Combo</p>
                        <p className="text-xl font-mono text-amber-500">x{combo}</p>
                    </div>
                </div>
            </div>
            ...
            {gameState === 'start' && (
                <div className="bg-slate-900/90 border-2 border-slate-800 p-10 rounded-3xl text-center backdrop-blur-md">
                    ...
                    <p className="text-slate-300 mb-8 leading-relaxed">
                        Eulalie, la partition défile. <br/>
                        L'audition va bientôt commencer. Prépare ton souffle.
                    </p>
                    <button onClick={prepareGame} className="bg-amber-600 hover:bg-amber-50 text-black px-10 py-4 rounded-full font-bold uppercase tracking-widest shadow-xl transition-all">
                        Entrer en scène
                    </button>
                </div>
            )}
            ...
            {(gameState === 'ready' || gameState === 'countdown' || gameState === 'playing' || gameState === 'failed') && (
                <div className="relative flex flex-col gap-6">
                    ...
                    <div className="flex justify-center gap-2 font-mono text-lg bg-black/60 p-4 rounded-2xl border border-slate-800">
                        {SEGMENTS.map((seg, i) => (
                            <span key={i} className={revealedSegments.includes(seg) ? "text-amber-500 animate-pulse" : "text-slate-800"}>
                                {revealedSegments.includes(seg) ? seg : "???"}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="relative h-[500px] bg-slate-950 rounded-3xl border-2 border-slate-900 overflow-hidden flex shadow-inner">
                        <div
                            className="absolute left-0 right-0 h-1 bg-red-600/80 shadow-[0_0_15px_rgba(220,38,38,0.7)] z-50 pointer-events-none"
                        ...
                        {lanes.map((lane, i) => (
                            ...
                            >
                                <div
                                    className={`absolute left-1/2 -translate-x-1/2 w-14 h-14 rounded-full border-2 flex items-center justify-center transition-all duration-75 z-10 pointer-events-none ${
                                        activeLanes[i]
                                            ? 'border-amber-400 bg-amber-400/20 scale-110 shadow-[0_0_20px_rgba(251,191,36,0.4)]'
                                            : 'border-slate-800 text-slate-700'
                                    }`}
                                    style={{ top: `${HIT_ZONE_Y}%`, transform: 'translate(-50%, -50%)' }}
                                >
                                    <span className={`text-xs font-black ${activeLanes[i] ? 'text-amber-400' : ''}`}>
                                        {lane.key}
                                    </span>
                                </div>
                                ...
                                {notes.filter(n => n.lane === i).map(n => (
                                    <div
                                        key={n.id}
                                        className={`absolute left-1/2 -translate-x-1/2 z-40 ${lane.color}`}
                                        style={{ top: `${n.y}%`, transform: 'translate(-50%, -50%)' }}
                                    >
                                        <div className="p-3 bg-slate-900 rounded-full border-2 border-current shadow-lg transform active:scale-95">
                                            {lane.icon}
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="bg-slate-900 border-2 border-amber-500 p-8 rounded-3xl text-center shadow-2xl animate-scaleIn">
                    ...
                    <form onSubmit={handleFinalSubmit} className="space-y-4 max-w-sm mx-auto">
                        ...
                        <input
                            ...
                            className={`w-full bg-slate-950 border-2 rounded-xl px-4 py-4 text-center text-white outline-none transition-all ${error ? 'border-red-500 animate-shake' : 'border-amber-500/30 focus:border-amber-500'}`}
                            placeholder="Ta réponse..."
                            autoFocus
                        />
                        <button type="submit" className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold py-4 rounded-xl uppercase text-sm shadow-lg transition-all">
                            Valider le secret
                        </button>
                    </form>
                </div>
